...
function FinanzasPanel({ token }) {
  ...
  const [activeTab, setActiveTab] = useState('cxp'); // 'cxp', 'cxc', 'aging', 'tc'
  ...
  const cargarCuentasPorPagar = React.useCallback(async () => {
    try {
      ...
      mostrarMensaje(`Error al cargar CxP: ${err.message}`, 'error');
    }
  }, []);
  ...
  const cargarCuentasPorCobrar = React.useCallback(async () => {
    try {
      ...
      mostrarMensaje(`Error al cargar CxC: ${err.message}`, 'error');
    }
  }, []);
  ...
  const cargarAging = React.useCallback(async () => {
    try {
      const data = await apiFacade.finanzas.aging.obtener();
      setAgingData(data);
    } catch (err) {
      console.error('Error cargando aging:', err);
    }
  }, []);

  const cargarTiposCambio = React.useCallback(async () => {
    try {
      const data = await apiFacade.finanzas.tipoCambio.listar();
      setTiposCambio(data);
    } catch (err) {
  ...
  }, [token, cargarCuentasPorPagar, cargarCuentasPorCobrar, cargarTransacciones, cargarAging, cargarTiposCambio]);
  ...
  const handleCreateCxP = async (e) => {
    ...
    try {
      ...
      setNewCxP({ proveedor: '', monto: '', fechaVencimiento: '', concepto: '', estado: 'pendiente' });
      setShowAddCxP(false);
      cargarCuentasPorPagar();
    } catch (err) {
  ...
  const handleCreateCxC = async (e) => {
    ...
    try {
      ...
      setNewCxC({ cliente: '', monto: '', fechaVencimiento: '', concepto: '', estado: 'pendiente' });
      setShowAddCxC(false);
      cargarCuentasPorCobrar();
    } catch (err) {
        ...
        <div className="modal-overlay" onClick={() => setShowAddCxP(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleCreateCxP}>
              ...
              <input 
                type="text"
                value={newCxP.proveedor}
                onChange={(e) => setNewCxP({...newCxP, proveedor: e.target.value})}
                required
              />
              <label>Monto *</label>
              <input 
              ...
              <input 
                type="text"
                value={newCxP.concepto}
                onChange={(e) => setNewCxP({...newCxP, concepto: e.target.value})}
                required
              />
        ...
        <div className="modal-overlay" onClick={() => setShowAddCxC(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleCreateCxC}>
              ...
              <input 
                type="text"
                value={newCxC.cliente}
                onChange={(e) => setNewCxC({...newCxC, cliente: e.target.value})}
                required
              />
              <label>Monto *</label>
              <input 
              ...
              <input 
                type="text"
                value={newCxC.concepto}
                onChange={(e) => setNewCxC({...newCxC, concepto: e.target.value})}
                required
              />
              <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1.5rem' }}>
                <button type="submit" className="btn btn--primary" disabled={loading} style={{ flex: 1 }}>
