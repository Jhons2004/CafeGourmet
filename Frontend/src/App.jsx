...
function App() {
  ...
  <button style={{ marginBottom: 16, width: '100%' }} className="btn" onClick={() => { setPanel('reportes'); loadKpis(); loadVentasDiarias(7); loadMerma(30); }}>Ir a Reportes</button>
  <button style={{ marginBottom: 16, width: '100%' }} className="btn" onClick={() => { 
    setPanel('finanzas'); 
    loadFinanzas(); 
    loadProveedores(); 
    loadOrdenes(); 
    ...
    loadAging();
    loadTC(false);
    // Defaults de vencimiento si están vacíos
    const toDateStr = (d)=> new Date(d.getTime() - d.getTimezoneOffset()*60000).toISOString().slice(0,10);
    if(!cxpForm.fechaVencimiento){ const d=new Date(); d.setDate(d.getDate()+30); setCxpForm(prev=> ({ ...prev, fechaVencimiento: toDateStr(d) })); }
    if(!cxcForm.fechaVencimiento){ const d=new Date(); d.setDate(d.getDate()+15); setCxcForm(prev=> ({ ...prev, fechaVencimiento: toDateStr(d) })); }
  }}>Ir a Finanzas</button>
  {permiteConfig && (
